...
async function run() {
  ...
  for (let i = 0; i < tabListItems.length; i++) {
    ...
    for (let j = 0; j < accordionHeader.length; j++) {
      ...
      const accordionDiv = await page.evaluateHandle(el => el.nextElementSibling, accordionHeader[j]);
      let d = await accordionDiv.$$("div > h3");
      for (let k = 0; k < d.length; k++) {
        console.log(
          "\t" + (await (await d[k].getProperty("innerText")).jsonValue())
        );
        const dNextSibling = await page.evaluateHandle(el => el.nextElementSibling, d[k]);
        console.log(await (await dNextSibling.getProperty("id")).jsonValue());
